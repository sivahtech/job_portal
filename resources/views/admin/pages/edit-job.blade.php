    ...
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                $job = $data['job'];
                @endphp
                <form action="{{route('update.job',['jobId'=>$job->id]) }}" class="add-job" method="post" enctype="multipart/form-data">
                    @csrf
                    @if(isset($job))
                    @method('PUT')
                    @endif
                    ...
                    <div class="form-group">
                        ...
                        <div class="adv-input">
                            <input type="text" id="job_title" name="job_title" class="form-control" placeholder="Senior UI/UX Designer" value="{{ old('title', $job->title ?? '') }}">
                        </div>
                        @error('job_title')
                        <p>{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="adv-input">
                            <select name="qualification[]" id="qualifications" class="form-control select-wrap" multiple placeholder="Select job ">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="adv-input">
                                    <input type="text" id="vacancies" name="vacancies" class="form-control" placeholder="03 Persons" value="{{  old('vacancies',$job->vacancies ??'') }}">
                                </div>
                                @error('vacancies')
                                <p>{{ $message }}</p>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="adv-input">
                                    <input type="text" id="exp" name="experience" class="form-control" placeholder="Type Experience" value="{{ old('experience',$job->experience ??'') }}">
                                </div>
                                @error('experience')
                                <p>{{ $message }}</p>
                                @enderror
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            ...
                            <div class="salary radio inline">
                                <label><input type="radio" name="salary_type" value="fixed" class="fixed" {{ isset($job) ? ($job->salary_type === 'fixed'?'checked':'') :''  }}>Fixed Salary</label>
                                <label><input type="radio" name="salary_type" value="range" class="salary-range" {{ isset($job) ? ($job->salary_type === 'range'?'checked':'') :''  }}>Salary Range</label>
                            ...
                            <div class="extra-fields">
                                <div class="adv-input conditional-field {{ isset($job) ? ($job->salary_type === 'fixed'?'show':'') :''  }}" id="fixed_salary">
                                    <input type="number" name="fixed_salary" class="form-control" placeholder="Salary" value="{{ $job->salary ?? '' }}">
                                </div>
                                <div class="inline extra-field conditional-field {{ isset($job) ? ($job->salary_type === 'range'?'show':'') :''  }}" id="salary_range">
                                    <div class="form-group">
                                        <div class="adv-input">
                                            <input type="number" name="min_salary" class="form-control" placeholder="Min Salary" value="{{ $job->salary ?? '' }}">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="adv-input">
                                            <input type="number" name="max_salary" class="form-control" placeholder="Max Salary" value="{{ $job->max_salary ?? '' }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="adv-input">
                            <input type="date" id="deadline" name="deadline" class="form-control" placeholder="DD-MM-YY" value="{{ old('deadline',$carbon->parse($job->deadline ?? '')->format('Y-m-d')) }}">
                        </div> @error('deadline')
                        <p>{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <div class="adv-input">
                            <textarea name="job_desc" id="job_desc" cols="30" rows="10" class="form-control" placeholder="Job Description">{{ old('job_desc',$job->description ?? '') }}</textarea>
                        </div>
                        @error('job_desc')
                        <p>{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        <div class="radio inline">
                            @isset($job)
                            <label><input type="radio" name="location" value="current_location" class=" " {{ old('location') == 'current_location'?'checked':'checked' }}>location</label>
                            <label><input type="radio" name="location" value="other_location" class=" " {{ old('location') == 'other_location'?'checked':'' }}>{{ isset($job) ? 'Update':'Other' }} location</label>
                        ...
                        <div class="d-flex justify-content-between {{($job)?(old('location') == 'other_location'?'':'d-none'):'' }} " id="locations-wrap">
                            <div class="form-group">
                    ...
                    <div class="form-group" id="status-wrap">
                        ...
                        <select class="form-select" id="status" name="status">
                            <option value="1" {{ 1 === old('status',$job->status ?? null) ?'selected':''  }}>Active</option>
                            <option value="2" {{ 2 === old('status',$job->status ?? null) ?'selected':''  }}>Deleted</option>
                        </select>
                    </div>
                    @endisset
                    <div class="form-group form-submit">
                        <input type="submit" class="btn btn-primary" value="{{ isset($job) ? 'Update Job' : 'Post Now' }}">
                    </div>
                </form>
            </div>
        </div>
